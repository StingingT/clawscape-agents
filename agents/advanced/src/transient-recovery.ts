...
export function settleAstraTransient(store:Store,command:ActionCommand,result:ActionResult,
  ...
  if(!['move','close_interface','style'].includes(command.intent.operation))return no('Operation requires attributable terminal evidence.');
  ...
  const action=agencyCandidate(before,{goal:command.plan_id,reason:'Observe existing transient intent',intent:command.intent});
  ...
  const settled:ActionResult={...result,status:'CANCELLED',reason:'RECONCILED_TRANSIENT_INTERRUPTED',at:now,evidence:[quiet.reason]};
  store.db.transaction(()=>{
    ...
    store.append('transient_reconciliation',command.action_id,{original:entry,at:now,reason:quiet.reason});
    store.result(settled);
  }).immediate();
  states.delete(command.action_id);
  return {result:settled,settling:false,reason:quiet.reason};
}
