 ...
 * Only known transient commands can be abandoned without asserting their original result. */
export function observeQuietStep(action: StepAction, before: LiveState, after: LiveState, now: number,
  ...
  if (!['walkTo', 'retreat', 'closeModal', 'closeShop', 'setCombatStyle'].includes(action.type))
  ...
  return { window, settled: now - window.since >= 30_000,
    reason: now - window.since >= 30_000 ? 'Transient step interrupted after a continuous quiet window; no success or replay inferred.' :
      'Observing a 30-second quiet window before retiring this transient step.' };
}
