...
test('mixed style and accidental protected XP are blocked before irreversible combat',()=>{
  const s=state(),d=chooseDevelopment(s,createMemory(identity,{combat:2}),1000,'ranged-magic');
...
test('pure strategy and parent-linked food preparation survive a restart and changed role suggestion',t=>{
  ...
  const p=a.plan(s);assert.ok(isSelection(p));assert.equal(p.decision.goal.id,'train-ranged');assert.equal(p.task.kind,'food');
