...
function githubLearning(state) {
  const inventory2 = state.inventory ?? [];
  if (build === "ranged-magic") {
    const arrows = [...inventory2, ...state.equipment ?? []].filter((item) => /arrow/i.test(text(item.name))).reduce((sum, item) => sum + (typeof item.count === "number" ? item.count : 1), 0);
    if (arrows === 0)
  ...
  if (role === "economy" && inventory2.length >= 28 && state.bank?.isOpen !== true) {
...
function safeAmmoSupplyCandidates(state) {
  if (build !== "ranged-magic")
    return [];
  const inventory2 = state.inventory ?? [];
  const hasAxe = inventory2.concat(state.equipment ?? []).some((i) => /axe/i.test(String(i.name)) && !/pickaxe/i.test(String(i.name)));
  if (!hasAxe) {
  ...
  if (shop.isOpen === true) {
    const sale = inventory2.find((item) => /logs?|shortbow|longbow|arrow shafts/i.test(text(item.name)) && typeof item.slot === "number");
    if (sale)
  ...
  const log = inventory2.find((item) => /logs?/i.test(text(item.name)) && typeof item.slot === "number");
  ...
  const knife = inventory2.find((item) => /^knife$/i.test(text(item.name)) && typeof item.slot === "number");
  if (log && knife) {
...
function ammoCandidates(state) {
  ...
  const arrows = [...state.inventory ?? [], ...state.equipment ?? []].filter((item) => arrowRank(String(item.name)) > 0 && arrowRank(String(item.name)) <= maxArrowRank).reduce((total, item) => total + (typeof item.count === "number" ? item.count : 1), 0);
  if (arrows >= 15)
    return [];
...
function coinsIn(inventory2) {
  return inventory2.filter((item) => /coins/i.test(text(item.name))).reduce((total, item) => total + (typeof item.count === "number" ? item.count : 1), 0);
}
...
function bankingCandidates(state) {
  const inventory2 = state.inventory ?? [];
  const bankableItems = inventory2.filter((item) => isBankableResource(String(item.name)) && shouldDepositAtBank(String(item.name), inventory2.length >= 28, isFood(item)));
  const coins = coinsIn(inventory2);
  const surplusCoins = Math.max(0, coins - workingCashReserve(state));
  const inventoryFull = inventory2.length >= 28;
  const recoveryCash = role !== "economy" && !inventory2.some(isFood) && !inventory2.some((i) => /small fishing net/i.test(String(i.name))) && coins < 5;
  const bank = state.bank ?? {};
  if (bank.isOpen === true) {
    work.bankItems = Array.isArray(bank.items) ? bank.items : [];
    saveWork();
    if (shouldCloseAfterFoodWithdrawal(work.foodWithdrawalPending === true, inventory2.some(isFood))) {
    ...
    const coinStack = inventory2.find((item) => /coins/i.test(text(item.name)) && typeof item.slot === "number");
    ...
    if (!inventory2.some(isFood) && food && inventory2.length < 28) {
      work.foodWithdrawalPending = true;
      saveWork();
      return [{ id: "withdraw-recovery-food", type: "bankWithdraw", fields: { slot: food.slot, amount: Math.min(BUILD.foodTripTarget, 28 - inventory2.length, Number(food.count)) }, waitTicks: 2 }];
    }
    const net = work.bankItems.find((i) => /small fishing net/i.test(String(i.name)));
    if (role !== "economy" && !inventory2.some(isFood) && !inventory2.some((i) => /small fishing net/i.test(String(i.name))) && net)
    ...
    if (role !== "economy" && !inventory2.some(isFood) && coins < 5 && bankCoins)
...
function gearCandidates(state) {
  ...
  if (build === "ranged-magic") {
    ...
    const bow = inventory2.filter((i) => bowQuality(i.id) > currentBowQuality).sort((a, b) => bowQuality(b.id) - bowQuality(a.id))[0];
    ...
    const arrows = inventory2.find(compatible);
  ...
  if (build !== "ranged-magic" && weaponName !== "unarmed" && !hasShield) {
    const shield = inventory2.find((item) => text(item.name).includes("shield"));
...
function combatLoadoutCandidates(state) {
  ...
  const twoHanded = inventory2.find((item) => /two-handed|2h|battleaxe|warhammer|halberd|godsword/i.test(text(item.name)));
  const options = [];
  ...
  if (!shieldEquipped) {
    const shield = inventory2.find((item) => /shield|defender/i.test(text(item.name)));
...
`);
    ...
    try {
      ...
      work.autonomy ??= {};
      const transientNavigation = action.id === "autonomy-map-loading" || result.navigation?.status === "loading-map" || result.navigation?.status === "replanning";
