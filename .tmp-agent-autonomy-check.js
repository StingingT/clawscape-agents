...
function ammoCandidates(state) {
  ...
  if (coins < minimumPurchaseBudget) {
    ...
    return pickpocket.length > 0 ? pickpocket : safeAmmoSupplyCandidates(state);
  }
...
function bankingCandidates(state) {
  const inventory = state.inventory ?? [];
  const bankableItems = inventory.filter((item) => isBankableResource(String(item.name)));
  ...
  if (bank.isOpen === true) {
    ...
    if (!inventory.some(isFood) && food && inventory.length < 28)
      return [{ id: "withdraw-recovery-food", type: "bankWithdraw", fields: { slot: food.slot, amount: Math.min(8, Number(food.count)) }, waitTicks: 2 }];
...
async function candidates(state) {
  ...
  if (build === "ranged-magic") {
    const recovery = available(nearbyAmmoRecovery(state));
    if (recovery.length)
      return recovery;
  }
  if (ammo.length > 0)
...
`);
    ...
    try {
      ...
      if (action.type === "closeModal" && state.bank?.isOpen === true && next.bank?.isOpen === false && work.bankReturn) {
