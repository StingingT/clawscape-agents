...
async function runScenario({blocked=false,unknown=false,deny=false,movement=false,mapWait=false,legacyNavigation=false,combatGoal=false,badStyle=false,meleeGoal=false,rebound=false,missingInput=false}={}){
  ...
  try{
    ...
    const environment={
      ...
      incidentalOpportunity:()=>undefined,urgentAgencyAction:()=>undefined,validateMetal:()=>true,validateBow:()=>true,validateFishing:()=>true,
...
async function runGatherScenario(){
  ...
  try {
    ...
    const env={agency,steps:11,character:'test',role:'resource',build:'melee',forumEnabled:false,incidentalOpportunity:()=>undefined,console:{log(){},error(){}},Date:TestDate,
